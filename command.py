...
def say(player,room,message):
    ...
    if message == '':
        u.send(player.conn,"Say what?",player.key)
...
def dig(player,message,rooms,cursor):
    ...
    try:
        ...
        u.send(player.conn,"Incorrect usage, try: dig [n,e,s,w,u,d] New_Name",player.key)
        return
...
def flee(player,message,rooms):
    ...
    if success:
        ...
        look(player,'',rooms)
        if player.target:
            u.send(player.conn,f"You successfully escaped {player.target.name}",player.key)
...
def kill(player, message, rooms):
    ...
    rooms[player.location].broadcast(f"{player.name} attacks {target.name}!",player,target)
...
def level(player,msg,cursor):
    ...
    if lvl == 0 or player.xp >= xpReq:
        ...
        u.send(player.conn,f"XP requirements not met, you need {xpReq - player.xp} more.",player.key)
        return
